...
class _DistributorStatisticsScreenState extends State<DistributorStatisticsScreen> with SingleTickerProviderStateMixin {
      ...
      builder: (context, snapshot) {
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildSummaryCard(totalMilk, totalSpent, snapshot.data!.docs.length),
  ...
  Widget _buildSummaryCard(double totalMilk, double totalSpent, int transactionCount) {
    return Card(
      elevation: 4,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            const SizedBox(height: 10),
            _buildStatRow(Icons.water_drop, 'Total Milk Collected', '${totalMilk.toStringAsFixed(2)} Litres'),
            const SizedBox(height: 10),
            _buildStatRow(Icons.currency_rupee, 'Total Amount Paid', '₹${totalSpent.toStringAsFixed(2)}'),
            const SizedBox(height: 10),
            _buildStatRow(Icons.receipt_long, 'Total Transactions', '$transactionCount'),
            const SizedBox(height: 10),
            _buildStatRow(Icons.calculate, 'Average Price Paid', '₹${(totalSpent / totalMilk).toStringAsFixed(2)}/L'),
          ],
        ),
      ),
    );
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      if (farmerSnapshot.hasData && farmerSnapshot.data!.exists) {
                        final farmerData = farmerSnapshot.data!.data() as Map<String, dynamic>;
                        _showTransactionDetails(data, farmerData, doc.id, dateString);
                      }
                    },
                  ),
                );
